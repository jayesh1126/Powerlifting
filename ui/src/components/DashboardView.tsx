...
export const rpeChart: RpeChart = {
  ...
  export const calculateAverageRpeByDate = (sets: SetModel[]) => {
  ...
  export const calculateVolumeByDate = (sets: SetModel[], exerciseName: string) => {
...
export const filterSetsByCategory = (sets: SetModel[], selectedCategory: string) => {
...
export const filterSetsForOneRepMax = (sets: SetModel []) => {
...
export const calculateOneRepMax = (setData: SetModel) => {
...
export const calculateAverageOneRepMax = (sets: SetModel[]) => {
  let totalOneRepMax = 0;
  let validSetsCount = 0;

  // Calculate 1RM for each set and accumulate
  for (const setData of sets) {
    const oneRepMaxResult = calculateOneRepMax(setData);
...
useEffect(() => {
  const squatEstimate = calculateAverageOneRepMax(squatOneRepMaxSets);
  setSquatOneRepMaxEstimate(squatEstimate);

  const benchEstimate = calculateAverageOneRepMax(benchOneRepMaxSets);
  setBenchOneRepMaxEstimate(benchEstimate);

  const deadliftEstimate = calculateAverageOneRepMax(deadliftOneRepMaxSets);
  setDeadliftOneRepMaxEstimate(deadliftEstimate);
